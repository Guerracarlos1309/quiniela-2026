document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateStandings(group) {
    ...
    const sortedTeams = Object.keys(standings).sort(
      (a, b) =>
        standings[b].pts - standings[a].pts || standings[b].g - standings[a].g,
    );
    ...
    try {
      ...
      if (res.ok) {
        ...
        document.getElementById("user-display-name").textContent =
          currentUser.name;
        document.getElementById("user-display-phone").textContent =
        ...
        if (currentUser.must_change_password) {
          document.getElementById("force-change-password-modal").style.display =
            "flex";
        }
        ...
        try {
          ...
          if (tabAllResults) {
            tabAllResults.style.display = showAllResults
              ? "inline-block"
              : "none";
          }
        } catch (err) {
        ...
        if (data.predictions && data.predictions.length > 0) {
          if (predictionsLocked) {
            document.getElementById("locked-badge").style.display =
              "inline-block";
            ...
            document.getElementById("read-only-badge").style.display =
              "inline-block";
            document.getElementById("edit-btn").style.display = "inline-block";
          }
          document.getElementById("download-pdf-btn").style.display =
            "inline-block";
  ...
  function showSection(type) {
    ...
    if (guestActions)
      guestActions.style.display = type === "auth" ? "block" : "none";

    // Show static rules section only for guests
    const rulesSection = document.getElementById("rules-section");
    if (rulesSection)
      rulesSection.style.display = type === "auth" ? "block" : "none";

    document.getElementById("main-content").style.display =
      type === "main" ? "block" : "none";
    if (type === "main") {
      setupUserTabs();
      startSystemPolling();
    } else {
    ...
    matchesByGroup[group].forEach((match, mIdx) => {
      ...
      if (matchPredictions.length === 0) {
        ...
        matchPredictions.forEach((pred) => {
          const result = calculateHitPoints(
            pred.score1,
            pred.score2,
            official?.score1,
            official?.score2,
  ...
  function renderLeaderboard(data) {
    ...
    data.sort((a, b) => b.points - a.points || b.exact_hits - a.exact_hits);
  ...
  document
    .querySelectorAll(".close-modal, .close-modal-btn, .close-rules-modal")
    .forEach((btn) => {
      btn.onclick = () => {
  ...
  if (forcePwSubmit) {
    forcePwSubmit.onclick = async () => {
      ...
      try {
        ...
        if (res.ok) {
          showMessage(
            "✅ Contraseña actualizada! Recargando...",
            "success",
            msgBox,
          );
  ...
  document.getElementById("edit-btn").onclick = () => {
    if (currentUser && !currentUser.is_active) {
      showMessage(
        "❌ Tu cuenta está en revisión. No puedes realizar cambios aún.",
        "error",
      );
      return;
    }
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (currentUser && !currentUser.is_active) {
      showMessage(
        "❌ Estás siendo revisado, espera hasta que activen tu cuenta",
        "error",
      );
      return;
    }
  ...
  document.getElementById("confirm-submit-btn").onclick = async () => {
    ...
    if (predictions.length === 0) {
      ...
      showMessage(
        "❌ No se detectaron partidos para enviar. Por favor recarga la página.",
        "error",
      );
  ...
  if (downloadPdfBtn) {
    downloadPdfBtn.onclick = () => {
      if (!window.jspdf || !window.jspdf.jsPDF) {
        showMessage(
          "❌ Error al cargar la librería de PDF. Por favor recarga la página.",
          "error",
        );
        return;
      }
      ...
      doc.setFontSize(12);
      doc.text(
        `Participante: ${currentUser ? currentUser.name : "Usuario"}`,
        14,
        32,
      );
      ...
      doc.save(
        `Quiniela_2026_${currentUser ? currentUser.name.replace(/\s+/g, "_") : "Usuario"}.pdf`,
      );
    };
  }
